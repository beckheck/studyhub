...
export default function TodaySchedule() {
  ...
  const isDuringSemester = (() => {
    ...
    const inRange = (start: string, end: string) => start && end && todayStr >= start && todayStr <= end
              ...
              {todayEvents.map((event, index) => {
                ...
                return (
                  <div key={event.id} className="flex items-center gap-2">
                    {index > 0 && <div className="w-4 h-px bg-zinc-300 dark:bg-zinc-600 shrink-0" />}
                    <button
                      ...
                      className={`
                        ...
                        ${isCurrent ? 'ring-2 ring-offset-1 ring-offset-white dark:ring-offset-zinc-900' : ''}
                        ${isPast ? 'opacity-50' : 'hover:scale-105 cursor-pointer'}
                      `}
                      style={{
